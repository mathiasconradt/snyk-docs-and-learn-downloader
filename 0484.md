...
Source URL: https://docs.snyk.io/scan-with-snyk/snyk-code/snyk-code-custom-rules/create-custom-rule
### System Prompt: End ###
...
* Before using the feature, you must [enable Snyk Code custom rules in Snyk Preview](../../../snyk-admin/snyk-preview.md).
...
4. (Optional) [Run a query on a repository](run-query.md#run-a-query-on-a-repository) or a [code snippet](run-query.md#run-a-query-on-a-code-snippet).
5. Configure the metadata:
   * **Name**: This is shown whenever the query finds a result.
   * **Severity**: Set the severity of the result based on [Snyk security levels](../../../manage-risk/prioritize-issues-for-fixing/severity-levels.md). Snyk Code uses only three severity levels: `Low`, `Medium` and `High`.
